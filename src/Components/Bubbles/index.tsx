...
export default function NavBar() {
  ...
  return <div id="bubblearea" className={styles.bubblearea}></div>;
}
...
function Bubbles() {
  ...
  numberofBubbles =
    minNoBubbles + Math.floor(Math.random() * (maxNoBubbles + 1));
  ...
  for (var i = 0; i < numberofBubbles; i++) {
    ...
    $element.setAttribute("class", styles.bubble + " primary");
    ...
    $element.setAttribute(
      "style",
      `-webkit-animation: movebubbles ${duration}s linear infinite, sideWays ${sideduration}s ease-in-out infinite alternate; opacity: ${opacity}; height: ${bubblesize}px; width: ${bubblesize}px; left: ${bubblePosition}px;`
    );
